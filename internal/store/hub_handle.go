...
type HubHandle struct {
	...
	BaseBranch  string `json:"base_branch,omitempty"`   // merge target resolved at Prepare (PR base / repo default); never a draft branch
...
func (db *DB) SaveHubHandle(h *HubHandle) error {
	...
	_, err := db.Exec(
		`INSERT OR REPLACE INTO hub_handles (task_id, backend, remote_id, idempotency_key, status, draft_branch, base_branch, base_head, anchor_head, deploy_key_id, created_at, updated_at)
		 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)`,
		h.TaskID, h.Backend, h.RemoteID, h.IdempotencyKey, h.Status, h.DraftBranch, h.BaseBranch, h.BaseHEAD, h.AnchorHEAD, h.DeployKeyID,
	)
	if err != nil {
...
func (db *DB) GetHubHandle(taskID int64) (*HubHandle, error) {
	var h HubHandle
	err := db.QueryRow(
		`SELECT task_id, backend, remote_id, idempotency_key, status, draft_branch, base_branch, base_head, anchor_head, deploy_key_id, created_at, updated_at
		 FROM hub_handles WHERE task_id = ?`, taskID,
	).Scan(&h.TaskID, &h.Backend, &h.RemoteID, &h.IdempotencyKey, &h.Status, &h.DraftBranch, &h.BaseBranch, &h.BaseHEAD, &h.AnchorHEAD, &h.DeployKeyID, &h.CreatedAt, &h.UpdatedAt)
	if err != nil {
...
func (db *DB) ListNonTerminalHubHandles() ([]*HubHandle, error) {
	...
	for rows.Next() {
		var h HubHandle
		if err := rows.Scan(&h.TaskID, &h.Backend, &h.RemoteID, &h.IdempotencyKey, &h.Status, &h.DraftBranch, &h.BaseBranch, &h.BaseHEAD, &h.AnchorHEAD, &h.DeployKeyID, &h.CreatedAt, &h.UpdatedAt); err != nil {
