# 任务清单（Phase 2.5 + Phase 2.6，已归档）

> **状态：已归档（2026-08-20）**  
> 现行任务清单见 [`../TASKS.md`](../TASKS.md)（仅保留未完成待办与新增规划）。  
> 本文归档 **Phase 2.5：配置自动化与首次用户体验（P0+P1 已落地）** 与 **Phase 2.6：git_sync 落地（v3.1）** 的完整交付记录。  
> 来源：[`docs/TASKS.md`](../TASKS.md) 于 2026-08-20 的状态。  
> 关联文档：  
> - Phase 2.5 详细方案 → [`../CONFIG-AUTOMATION.md`](20260814-CONFIG-AUTOMATION.md)  
> - Phase 2.6 三阶段方案 → [`../20260815-git-sync-3phase-plan.md`](20260815-git-sync-3phase-plan.md)  
> - Phase 2.6 审核报告 → [`../20260820-phase2.6-review.md`](20260820-phase2.6-review.md)

---

## Phase 2.5：配置自动化与首次用户体验 ✅（P0+P1+P2 全部落地）

> 目标：让 MATEa 配置从「填表」变成「向导」，从 70 个字段变成 15 行 + 点几下。  
> 详细方案：[CONFIG-AUTOMATION.md](20260814-CONFIG-AUTOMATION.md)（细化版）  
...
### P1 ✅ 已完成（phase2.5/config-automation）
...
- [x] **C-18 一键连接测试（Gitea/LLM/Hub 后端状态灯）**
  Dashboard 或 SystemConfig 顶部显示各组件健康状态（`HealthStatus.vue` 状态灯 + 详情面板）。

- [x] **C-19 健康检查面板**
  汇总 Gitea / LLM / Hub 后端 / Deliver / DB 状态（`GET /api/health/summary`，逐组件实探）。

- [x] **C-20 配置导入/导出（JSON）**
  导出 `system_config` 扁平配置为 JSON（**默认脱敏**；`?include_secrets=1` 显式开关才含真实密钥，导出动作落审计）；
  导入复用 `applyConfigEntry`，`********` 占位符视为「保持当前值」，`UpdateBatch` 全量校验后原子应用。`GET /api/config/export` + `POST /api/config/import`，前端「备份与恢复」Tab（含确认框与密钥开关）。（2026-08-21 评审后修正：默认脱敏 + 审计 + 原子导入）

- [x] **C-21 环境变量自动发现与提示**
  启动时/向导中检测 `GITEA_URL`、`GITEA_ADMIN_TOKEN`、`GITEA_WEBHOOK_SECRET`、`OPENAI_API_KEY`、`DEEPSEEK_API_KEY`、`ANTHROPIC_API_KEY`、`SENSENOVA_API_KEY`、`OPENCODE_URL`、`HERMES_URL`，向导提供「从环境变量导入配置」一键吸入（值不回显）。

- [x] **C-22 文件系统可用空间预警**
  启动时 `sysinfo.CheckDisk` 检查 `data/` 所在分区，已用 ≥85% 时打印日志告警；`HealthStatus` 磁盘组件显示进度条与告警，Dashboard 顶部横幅提示。

---

## Phase 2.6：git_sync 落地（v3.1：A0 前置验证 + 共存窗口 + B2 拆细）✅（已归档）
...
> 审核报告：[20260820-phase2.6-review.md](20260820-phase2.6-review.md) — 22 项勾选 21 项完全吻合、1 项文案偏差已修正。  
...
- [x] **C1 删除 `mcp` transport（仅 workspace transport 用途）** ✅（2026-08-19）
  保留 `internal/agent/tools_mcp.go` 的 agent MCP tools 能力（与 workspace transport 无关），手术式删除：`WorkspaceTransportMCP` 常量移除；`ValidWorkspaceTransports()` 收敛为仅 `[git_sync]`（与早已拒绝 mcp 的 `IsWorkspaceTransportValid` 消除不一致——此前前者仍列 mcp，WebUI/校验两侧语义分裂）；`mcp` 与 `shared_path` 等无效 transport 在启动期校验中 fail loud，报错为通用拒绝文案 `workspace_transport %q is not supported (valid: git_sync)`（含迁移指引：唯一可用 transport 为 `git_sync`，Phase 3.9 计划回归 mcp）。测试 pin：`shared_path`/`mcp` 均不得在 valid 列表、`mcp` 配置仍 fail loud、`ValidWorkspaceTransports` len=1。全量 17 包 PASS。
